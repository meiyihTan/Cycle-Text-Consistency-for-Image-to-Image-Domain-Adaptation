...
import torchvision
from unet_seg import gray_TA_SP_CA_seg_UNet
...
checkpoint_dir = './IC15_004_results/result_IC15_baseline_gray_seg_TA_SP_CA_msssim_text_detection/'
result_dir = checkpoint_dir
ckpt = checkpoint_dir + 'final_model.pth'
...
unet = gray_TA_SP_CA_seg_UNet()
unet.load_state_dict(torch.load(ckpt))
unet.to(device)

if not os.path.isdir(result_dir + 'final_result/'):
    os.makedirs(result_dir + 'final_result/')
...
with torch.no_grad():
    ...
    for test_id in test_ids:
        ...
        Image.fromarray((output *255).astype('uint8')).convert('RGB').save(result_dir + 'final_result/%d.jpg' % (test_id))
