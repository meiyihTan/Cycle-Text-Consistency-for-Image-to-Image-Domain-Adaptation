#edge input image here is en_edge dataset get from dataset folder
...
import utils
from unet_seg import gray_TA_CA_edge_seg_UNet
from dataset.ICDAR15 import ICDAR15Dataset_edge_seg
...
checkpoint_dir = './IC15_004_results/result_IC15_baseline_gray_edge_seg_TA_CA_msssim_text_detection_text_det_loss/'
...
dataset = ICDAR15Dataset_edge_seg(list_file = list_file, root_dir = './dataset/', ps=ps)
...
for epoch in tqdm(range(lastepoch, 4001)):
    ...
    for sample in iter(dataloader):
        ...
        enh_out,seg_out = unet(in_imgs, in_gray_imgs, in_edge_imgs) 
        ...
        #loss =2.75*mae_loss+0.52*seg_loss+ 0.25*ms_ssim_loss+0.75*text_loss 
        loss =2.75*mae_loss+0.25*seg_loss+ 0.075*ms_ssim_loss+0.57*text_loss 
        loss.backward()
        G_opt.step()
