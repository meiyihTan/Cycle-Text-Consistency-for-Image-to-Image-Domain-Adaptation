...
checkpoint_dir = './IC15_004_results/train_IC15_gray_allTextSeg_TA_CA_msssim_mulAlphaLoss_text_detection_loss/'
...
for epoch in tqdm(range(lastepoch, 4001)):
    ...
    for sample in iter(dataloader):
        ...
        input_mul_seg_imgs = torch.squeeze(input_mul_seg_imgs.permute(0, 2, 3, 1)).cpu().detach().numpy()
        alpha_seg_imgs = alpha_seg_imgs.cpu().detach().numpy()
        gt_imgs_for_overlay =torch.squeeze(gt_imgs.permute(0, 2, 3, 1)).cpu().detach().numpy()        
        
        mul_pred = np.multiply(torch.squeeze(enh_out.permute(0, 2, 3, 1)).cpu().detach().numpy(), input_mul_seg_imgs)
        ...
        rgba_mul_pred = torch.from_numpy(rgba_mul_pred ).permute(0,3,1,2).to(device) 
        rgba_mul_gt = torch.from_numpy(rgba_mul_gt).permute(0,3,1,2).to(device)
        print(f'rgba_mul_pred : {rgba_mul_pred.shape},{rgba_mul_pred.max()},{rgba_mul_pred.min()},{type(rgba_mul_pred)}')
        print(f'rgba_mul_gt : {rgba_mul_gt.shape},{rgba_mul_gt.max()},{rgba_mul_gt.min()},{type(rgba_mul_gt)}')        
        
        mae_loss_overlay = utils.MAELoss(rgba_mul_pred, rgba_mul_gt )
        ...
        loss =2.75*mae_loss+0.25*seg_loss+ 0.075*ms_ssim_loss+0.57*mae_loss_overlay +0.57*text_loss 
        loss.backward()
        G_opt.step()
        ...
        torchvision.utils.save_image(rgba_mul_pred , result_dir + '%04d/%05d_trytryrgba_mul_pred_train.png' % (epoch, sample['ind'][0]))
        torchvision.utils.save_image(rgba_mul_gt , result_dir + '%04d/%05d_trytryrgba_mul_gt_train.png' % (epoch, sample['ind'][0]))
