...
def get_tl_line_values(line,LTRB=True,withTranscription=False,withConfidence=False,imWidth=0,imHeight=0):
    ...
    if LTRB:
        ...
        if withTranscription and withConfidence:
            cors = line.split(',')
            assert(len(cors)%2 -2 == 0), 'num cors should be even (with trans and conf).'
            try:
            ...
            cors = line.split(',')
            assert(len(cors)%2 -1 == 0), 'num cors should be even (with conf).'
            try:
            ...
            cors = line.split(',')
            assert(len(cors)%2 -1 == 0), 'num cors should be even (with trans).'
            try:
                points = [ float(ic) for ic in cors[:-1]]
            except Exception as e:
                raise(e)
...
def main_evaluation(p,default_evaluation_params_fn,validate_data_fn,evaluate_method_fn,show_result=True,per_sample=True):
    ...
    evalParams = default_evaluation_params_fn(p['flag'])
    if 'p' in p.keys():
